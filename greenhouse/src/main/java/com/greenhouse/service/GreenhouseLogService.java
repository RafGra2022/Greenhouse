...
public class GreenhouseLogService {
	...
	@Transactional
	public String logDevice(DeviceStatusRequest deviceStatusRequest) {
		...
		if (greenhouseDeviceLog == null) {
			throw new NotFoundInDatabaseGreenhouseException(
					"greenhouse device log entity is empty, try run request again with deviceWorking : true value");
		}
		greenhouseDeviceLog.setDateTo(LocalDateTime.now());
		greenhouseDeviceLog.setWorkTime(
				greenhouseDeviceLog.getDateFrom().until(greenhouseDeviceLog.getDateTo(), ChronoUnit.MINUTES));
		return "device logged";
	}
	...
	public PowerUsage getDevicePowerUsageReport() {
		LocalDateTime dateFrom = LocalDateTime
				.of(LocalDate.of(LocalDate.now().getYear(), LocalDate.now().getMonth(), 1), LocalTime.MIN);
		...
		List<DayPowerUsage> powerLogs = logs.stream().map(log -> {
			return new DayPowerUsage(log.getDateFrom().toLocalDate().toString(),
					log.getWorkTime() == null ? 0 : log.getWorkTime().intValue());
		}).toList();

		Map<String, Integer> powerLogsMap = powerLogs.stream().collect(Collectors
				.groupingBy(powerLog -> powerLog.date(), Collectors.summingInt(powerLog -> powerLog.workTime())));

		List<String> powerLogsSortedKey = powerLogsMap.keySet().stream().map(powerLog -> LocalDate.parse(powerLog))
				.sorted().map(powerLog -> powerLog.toString()).toList();

		return new PowerUsage(powerLogsSortedKey.stream().map(powerLogKey -> {
